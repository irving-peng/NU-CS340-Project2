...
class Streamer:
    def __init__(self, dst_ip, dst_port, src_ip=INADDR_ANY, src_port=0):
        ...
        self.receiveBuffer = {}
        self.dataQueue = []
        self.unacked_packets = {}  # Tracks all unacknowledged packets
        self.ack_lock = Lock()
        self.retransmit_timer = None
        self.retransmit_interval = 0.1  # Set lower for faster retry
        self.window_size = 10  # Start with a smaller window size
        self.retry_count = {}  # Track retries for each packet
        self.max_retries = 5  # Max retries for each packet
        self.lock = Lock()
    ...
    def _retransmit_packets(self):
        with self.ack_lock:
            ...
            if unacked:
                # Batch retransmission for improved performance
                for seq_num in unacked:
                    if self.retry_count[seq_num] < self.max_retries:
                        ...
                        del self.unacked_packets[seq_num]  # Drop packet if max retries exceeded
                        del self.retry_count[seq_num]

            # Dynamic window adjustment
            if len(unacked) < self.window_size // 2:
                self.window_size = min(self.window_size + 1, 50)  # Gradually increase window size up to a maximum
            else:
                self.window_size = max(self.window_size - 1, 10)  # Decrease window size if network congestion detected
        ...
        while not self.closed:
            try:
                ...
                if received_hash != expected_hash:
                    ...
                    continue  # Discard corrupted packet without ACK
